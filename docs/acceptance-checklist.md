...
>
> **2026-08-09 更新：阶段二核心链路和边界场景均已在 Capture 16.6 上完成。**
> 已覆盖普通网络、复用模块同名网络、全局网络、未命名网络、总线、跨页连接器
> 和多单元器件；结果摘要见本文记录区和 `MIGRATION.md`。
...
| 5.3 400 恢复 | | |
| 6.2 token 轮换 | | |
| 2.x flat-net 普通/层次/全局网络 | 通过 | `REF_INPUT`、`PD_DC_N_1`、`VCC_+12V` |
| 2.x 未命名网络 | 通过 | 2 个端点与人工结果完全匹配 |
| 2.x 总线 | 通过 | 72 成员、144 端点，无缺失 |
| 2.x 跨页连接器 | 通过 | 两侧端点与人工结果完全匹配 |
| 2.x 多单元器件 | 通过 | 所选单元 16/16 已连接引脚，物理引脚无重复 |
