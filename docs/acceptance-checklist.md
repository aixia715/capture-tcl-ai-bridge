...
- [ ] **0.1** 确认 Capture 版本与 Tcl 版本。在 Capture 的 Tcl 命令行执行：
...
- [ ] **0.3** 比对部署文件与仓库文件的 SHA-256 一致：

      Get-FileHash .\captureAiBridge.tcl, "<CaptureTclTarget>\captureAiBridge.tcl" |
          Select-Object Hash, Path

- [ ] **0.4** 再跑一次 `.\install.ps1`，确认幂等：manifest 不变、文件不被重写。

## 阶段 1：加载与显式启动

- [ ] **1.1** 在 Capture Tcl 命令行：
...
- [ ] **1.3** 确认 `%TEMP%\capture_tcl_bridge.json` 已生成，且 `capturePid`
      等于当前 Capture 进程 PID。**不要把 token 抄到任何地方。**

- [ ] **1.4** 再执行一次 `CaptureAiBridgeStart`，确认不会起第二个服务。
...
- [ ] **2.2** 确认迭代器耗尽时 `Next` 返回空字符串，且释放方法确实叫 `delete`。
...
- [ ] **2.4** 确认 pin/port occurrence 的父对象访问器名字。假设是
...
- [ ] **3.2** 多行 + UTF-8 脚本，确认返回值与中文原样往返。

- [ ] **3.3** **`puts` tee**：提交 `puts "hello-from-bridge"`，确认这行
      **同时**出现在 Capture 控制台和客户端 stdout。这是核心保证。

- [ ] **3.4** 故意失败的 Capture API 调用，确认返回 `errorInfo`、`errorCode`；
      `errorLine` 按 0.1 的版本决定期望（8.6 有精确行号，8.4 为 `null`）。

- [ ] **3.5** 依次跑通四个只读示例：`selected_refs`、`list_components`、
...
- [ ] **4.1** `set_component_value.tcl`：确认只改唯一命中的那个 occurrence，
      输出 before/after，且回读一致。**先记下 before 值以便还原。**

- [ ] **4.2** 重复位号时确认修改次数为 0。

- [ ] **4.3** `mark_selected_suffix.tcl`：选中若干器件执行，确认 Value 追加 `*`；
      **再执行一次**，确认不会变成 `**`（幂等）。

- [ ] **4.4** `remove_selected_suffix.tcl`：确认只去掉一个尾部 `*`。

- [ ] **4.5** 确认设计**没有被自动保存**（标题栏仍显示未保存），
      并确认没有调用 `RefreshParts`。

- [ ] **4.6** 把改动还原到测试前的值，或直接关闭不保存。

## 阶段 5：并发、超时与故障恢复

- [ ] **5.1** 提交一条长脚本，在它执行期间提交第二条，确认稳定返回 `BRIDGE_BUSY`。

- [ ] **5.2** 提交一条超过 30 秒的脚本，确认：HTTP 返回 504、**脚本没有被取消**、
      随后 `GET /v1/commands/<id>` 能取到完成结果。

- [ ] **5.3** **HTTP 400 恢复**（只在你同意的本机测试会话做）。在 Capture 控制台：
...
- [ ] **5.4** 跑一个会产生大结果或复杂 `errorInfo` 的脚本，确认不出现连续 400 刷屏，
      且下一条正常命令仍可执行。

- [ ] **5.5** 含反斜杠的输出（这是修掉的那个 JSON 缺陷）：

      puts "C:\\cadence\\SPB_16.6"

      确认正常返回，**不会**触发 `INVALID_RESULT`。

- [ ] **5.6** 在 pending / 故障状态下执行 `CaptureAiBridgeStop`，确认能输入且能收敛。

- [ ] **5.7** 只在需要取证时才执行 `CaptureAiBridgeDumpPendingResult <path>`，
      确认文件内容**不含 token**。看完自行决定是否删除。

## 阶段 6：生命周期

- [ ] **6.1** `CaptureAiBridgeStop`，确认描述文件消失、服务进程结束。

- [ ] **6.2** Stop → Start，比较前后描述文件里的 token，**必须不同**。

- [ ] **6.3** 退出 Capture，确认 8767 端口无监听、服务 PID 已结束、描述文件已清理：

      Get-NetTCPConnection -LocalPort 8767 -State Listen -ErrorAction SilentlyContinue
      Test-Path "$env:TEMP\capture_tcl_bridge.json"

- [ ] **6.4** 重启 Capture，确认必须重新 `source` 才能再次 Start。

## 阶段 7：卸载

- [ ] **7.1** `.\uninstall.ps1`，确认三个运行文件和 manifest 都消失，
