...
如果桥正在运行，先 `AiBridge stop`，再 `source`，再 `AiBridge start`。
...
1. `AiBridge status` 看是否已进入 polling-halted 状态；
2. 需要取证时，显式导出待回传的结果：
...
3. `AiBridge stop` 然后 `AiBridge start`。Stop→Start 会重置
...
`AiBridge stop` 是异步的。`AiBridge status` 显示 `stopping`
表示正在等服务端确认；显示需要清理时，再执行一次 `AiBridge stop` 重试。
